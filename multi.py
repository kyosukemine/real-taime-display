...
def update():
    global curves, T, V, cnt, AiChannels, now, old, start
    now = time()
    # for i in range(AiChannels):
    #     curves[i].setData(V[i][min(0, cnt - TIMERANGE):], T[min(0, cnt - TIMERANGE):])
    ...
    if (AiRange == caio.P20MA) or (AiRange == caio.P4TO20MA) or (AiRange == caio.PM20MA):
        ...
        # print("チャネル\t電圧")
        cnt += 1
        T = np.append(T, time() - start)
        for i in range(AiChannels):

            V[i] = np.append(V[i], AiData[i])
